        ...
        {user ? (
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-4 pr-4 border-r border-black/10 h-7">
              ...
              <div className="relative" ref={dropdownRef}>
                <div 
                  ...
                  className={`w-7 h-7 rounded-full border border-black/10 flex items-center justify-center bg-black/5 cursor-pointer hover:bg-black/10 transition-all ${isProfileOpen ? 'ring-2 ring-black/20' : ''}`}
                >
                      ...
                      {menuItems.map((item, idx) => (
                        <div key={item.label}>
                          ...
                          >
                            ...
                            <item.icon className={`w-3.5 h-3.5 opacity-30 group-hover:opacity-100 transition-opacity ${item.isDanger ? 'text-red-400' : 'text-brand-primary'}`} />
                          </button>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={onOpenDeposit} className="bg-brand-yellow text-black font-black px-4 h-8 rounded-full text-[11px] uppercase italic shadow-md hover:scale-105 hover:brightness-110 active:scale-95 transition-all">
                DEPOSIT
              </button>
            </div>
          </div>
