...
import { Search, Mail, User, ChevronDown, LogOut, History, Wallet, ArrowUpRight, ArrowDownLeft, FileText, Settings } from 'lucide-react';
...
export default function Header({ onAuth, onSignOut, user, authLoading = false, onOpenDeposit, onOpenBetsHistory, onOpenCheckTicket }: HeaderProps) {
  ...
  return (
    <header className="h-[60px] bg-brand-primary flex items-center px-4 sticky top-0 z-[100] shadow-md border-b border-black/5">
          ...
          <div className="max-w-[280px] flex-1">
            <div className="relative group">
              <input 
                type="text" 
                placeholder="Search" 
                className="w-full bg-white/95 rounded-full py-1.5 px-5 pr-10 text-[13px] text-black placeholder:text-gray-400 focus:outline-none shadow-[inset_0_1px_2px_rgba(0,0,0,0.05)] font-medium"
              />
      ...
      <div className="hidden lg:flex items-center gap-5">
        {user ? (
          <div className="flex items-center gap-5">
            <div className="flex items-center gap-5 pr-5 border-r border-black/10 h-8">
              {/* Mail Icon */}
              <div className="relative cursor-pointer hover:scale-110 transition-transform group">
                <div className="w-8 h-8 rounded-full border border-black/10 flex items-center justify-center bg-black/5 group-hover:bg-black/10">
                  <Mail className="w-4 h-4 text-black" />
                </div>
                <div className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-red-500 rounded-full border border-brand-primary flex items-center justify-center">
              ...
              <div className="relative" ref={dropdownRef}>
                <div 
                  ...
                  className={`w-8 h-8 rounded-full border border-black/10 flex items-center justify-center bg-black/5 cursor-pointer hover:bg-black/10 transition-all ${isProfileOpen ? 'scale-110 ring-2 ring-black/20' : ''}`}
                >
                ...
                <AnimatePresence>
                  {isProfileOpen && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-3 w-64 bg-[#1e1b21] rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/5 overflow-hidden py-1 z-[110]"
                    >
                      {menuItems.map((item, idx) => (
                        <div key={item.label}>
                          <button
                            ...
                            className={`w-full px-5 py-4 text-left text-[11px] font-black uppercase italic tracking-wider transition-colors flex items-center justify-between group ${
                              item.isDanger ? 'text-red-400 hover:bg-red-500/10' : 'text-gray-200 hover:bg-white/5 hover:text-white'
                            }`}
                          >
                            <span>{item.label}</span>
                            <item.icon className={`w-4 h-4 opacity-30 group-hover:opacity-100 transition-opacity ${item.isDanger ? 'text-red-400' : 'text-brand-primary'}`} />
                          </button>
              ...
              <div className="flex items-center gap-1 cursor-pointer hover:scale-105 transition-transform">
                <div className="w-5 h-3 bg-white/20 rounded-sm flex items-center justify-center overflow-hidden border border-black/10">
                  <img 
                ...
                <span className="text-black font-black text-[12px] uppercase italic">EN</span>
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={() => onAuth('login')}
              className="px-6 py-2.5 text-black font-black text-sm uppercase hover:bg-black/10 rounded-full transition-all italic"
            >
              Login
            </button>
            <button 
              onClick={() => onAuth('register')}
              className="bg-black text-brand-primary px-8 py-2.5 rounded-full text-sm font-black uppercase italic shadow-lg hover:scale-105 active:scale-95 transition-all"
            >
              Sign Up
            </button>
          </div>
