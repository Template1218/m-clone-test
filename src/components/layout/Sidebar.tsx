import { Trophy, ChevronRight, ChevronDown, Globe, Clock } from 'lucide-react';
import { TOP_LEAGUES, SPORTS } from '../../data';
        ...
        <div className="bg-brand-surface rounded-[24px] overflow-hidden border border-brand-border/50 shadow-xl">
          ...
          <div className="py-2">
            {TOP_LEAGUES.map((league) => (
              <div key={league.name} className="sidebar-item group !py-2 border-b border-brand-border/10 last:border-0 hover:bg-white/[0.02]">
                <div className="w-5 h-5 rounded-full bg-white/5 flex items-center justify-center flex-shrink-0 group-hover:bg-brand-primary/20 transition-colors border border-white/5">
                  <span className="text-[10px] leading-none">⚽</span>
                </div>
                <span className="truncate flex-1 text-[13px] font-semibold group-hover:text-white transition-colors">{league.name}</span>
                <ChevronRight className="w-4 h-4 text-gray-600 transition-all group-hover:text-brand-primary group-hover:translate-x-0.5" />
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="absolute top-full left-0 right-0 mt-2 bg-[#1e1a2b] border border-white/5 shadow-2xl rounded-xl py-2 z-50 animate-in fade-in zoom-in-95 origin-top">
              {['All Time', '1 Hour', '3 Hours', '6 Hours', '12 Hours', '24 Hours', '3 Days'].map((f) => (
                <button
                  ...
                  className={`w-full text-left px-6 py-2.5 text-[11px] font-black uppercase italic transition-colors hover:bg-white/5 flex items-center justify-between ${timeFilter === f ? 'text-brand-primary' : 'text-gray-400'}`}
                >
                  <span>{f}</span>
              ...
              return (
                <div key={sport.id} className="flex flex-col border-b border-brand-border/10 last:border-0">
                  <div 
                    ...
                    className={`sidebar-item group py-3.5 cursor-pointer hover:bg-white/[0.02] ${isExpanded ? 'bg-white/[0.03] text-white' : ''}`}
                  >
                    <div className="w-6 h-6 rounded-lg bg-white/5 flex items-center justify-center flex-shrink-0 group-hover:bg-brand-primary/10 transition-colors border border-white/5">
                      <span className="text-[14px] leading-none">{sportIcons[sport.id] || '⚽'}</span>
                    </div>
                    <span className="flex-1 font-black text-[12px] uppercase tracking-wide">{sport.name}</span>
                  ...
                  {isExpanded && sport.countries && (
                    <div className="bg-black/30 py-1 transition-all">
                      {sport.countries.map((country) => (
                        <div key={country.name} className="flex items-center gap-3 px-6 py-2.5 hover:bg-white/5 cursor-pointer text-[12px] text-gray-400 hover:text-white transition-colors group/country">
                          <Globe className="w-3.5 h-3.5 text-gray-600 group-hover/country:text-brand-primary/60 transition-colors" />
