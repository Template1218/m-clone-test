...
export default function AccountPanelModal({
  ...
  const {
    ...
    isFetching: slipsFetching,
  } = useMyUserBetslips(isOpen && (tab === "bets" || tab === "ticket"));
  const {
    data: ticket,
    ...
    submittedTicketId,
    isOpen && tab === "ticket" && !!submittedTicketId,
  );

  const isTicketSearching =
    ticketLoading || (ticketFetching && !!submittedTicketId);
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="bg-brand-dark/30 p-5 border border-white/5 rounded-xl">
                      <span className="text-gray-500 text-[9px] font-bold uppercase tracking-widest block mb-1">
                        Account ID
                      </span>
                ...
                <div className="space-y-3">
                  <div className="flex justify-end mb-2 px-1">
                    ...
                    >
                      <RotateCw
                        size={12}
                        className={slipsFetching ? "animate-spin" : ""}
                      />
                  ...
                  {slipsLoading ? (
                    ...
                    <div className="py-20 text-center text-gray-500 font-bold uppercase tracking-widest text-[11px]">
                      No bets found
                    </div>
                  ) : (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-6">
                            <div className="text-right">
                              <div className="text-[8px] uppercase font-bold text-gray-600 tracking-wider">
                                Stake
                              </div>
                              <div className="text-xs font-bold text-white">
                                {amount(slip.stake).toFixed(2)}
                              </div>
                            </div>
                            <div
                              className={`px-3 py-1.5 rounded-lg border ${config.bg} ${config.text} ${config.border} text-[9px] font-black tracking-widest`}
                            >
                              {config.label}
                            </div>
                          </div>
                  ...
                  {slips.length > slipsPerPage && (
                    <div className="flex items-center justify-between pt-2 px-1">
                      <div className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">
                        Page {Math.min(Math.max(1, slipPage), totalSlipPages)}{" "}
                        of {totalSlipPages}
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                        ...
                        <button
                          onClick={() =>
                            setSlipPage((p) => Math.min(totalSlipPages, p + 1))
                          }
                ...
                <div className="space-y-6">
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="bg-brand-primary text-black px-8 py-3.5 font-black text-[11px] uppercase rounded-xl tracking-widest shadow-lg shadow-brand-primary/10"
                    >
                      CHECK
                    </button>
                  </div>
                  ...
                  {isTicketSearching ? (
                    ...
                    <div className="text-rose-400 text-center py-10 font-bold uppercase tracking-widest text-[10px]">
                      Ticket not found.
                    </div>
                  ) : ticket ? (
                    <div className="space-y-4">
                      <div className="bg-brand-dark/40 border border-white/5 rounded-2xl overflow-hidden">
                        <div className="px-6 py-3 border-b border-white/5 flex justify-between items-center bg-white/5">
                          <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest italic leading-none">
                            Ticket Selections
                          </span>
                          <button
                          ...
                          >
                            <RotateCw
                              size={12}
                              className={ticketFetching ? "animate-spin" : ""}
                            />
                        ...
                        {(ticket.BetSelections || []).map((sel: any) => {
                          const outcomeName =
                            sel.snapshot?.outcome?.name ||
                            sel.Outcome?.name ||
                            "-";
                          const marketName =
                            sel.snapshot?.market?.name ||
                            sel.Outcome?.Market?.name ||
                            "-";
                          ...
                          return (
                            ...
                            >
                              <div>
                                <div className="font-bold text-white text-sm italic">
                                  {sel.snapshot?.fixture?.homeTeamName} v{" "}
                                  {sel.snapshot?.fixture?.awayTeamName}
                                </div>
                              ...
                              <div className="text-right flex flex-col items-end gap-1">
                                <div className="text-brand-primary font-black text-xs">
                                  {amount(sel.oddsAtPlacement).toFixed(2)}
                                </div>
                                <div
                                  className={`text-[7px] font-black uppercase px-2 py-0.5 border rounded ${config.bg} ${config.text} ${config.border}`}
                                >
                                  {config.label}
                                </div>
                              </div>
                        ...
                        <div className="bg-brand-dark/40 p-4 border border-white/5 rounded-xl">
                          ...
                          <div className="text-white font-black text-sm">
                            {(
                              ...
                              Math.max(amount(ticket.stake), 1)
                            ).toFixed(2)}
                          </div>
                        </div>
                        <div className="bg-brand-dark/40 p-4 border border-brand-primary/20 rounded-xl">
                          <div className="text-[8px] font-bold text-brand-primary uppercase tracking-widest">
                            Win
                          </div>
                          <div className="text-brand-primary font-black text-sm">
                            {fmtMoney(ticket.potentialPayout)}
                          </div>
                        </div>
                        <div className="bg-brand-dark/40 p-4 border border-white/5 rounded-xl">
                          <div className="text-[8px] font-bold text-rose-500 uppercase tracking-widest">
                            Tax
                          </div>
                          <div className="text-rose-400 font-black text-sm">
                            {fmtMoney(amount(ticket.potentialPayout) * 0.15)}
                          </div>
                        </div>
                      </div>
                    </div>
