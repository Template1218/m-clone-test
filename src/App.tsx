...
export default function App() {
  ...
  const toggleBet = (match: Match, market: string, selection: string, odd: number) => {
    setSelectedBets(prev => {
      ...
      const newBets = [...prev, { matchId: match.id, matchName: `${match.homeTeam} v ${match.awayTeam}`, market, selection, odd }];
  ...
  const removeBet = (matchId: string, market: string, selection: string) => {
    setSelectedBets(prev => prev.filter(b => !(b.matchId === matchId && b.market === market && b.selection === selection)));
  };
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col bg-brand-dark">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {(view === 'home' || view === 'detail' || view === 'live') && <Sidebar />}

        <main className={`flex-1 overflow-y-auto ${isGamesView || isVirtualView ? 'bg-[#0a0a0a]' : 'p-0 lg:p-4'} pb-32 lg:pb-4`}>
          {isGamesView ? (
            <GamesView />
          ) : isLiveView ? (
            <LiveView />
            ...
            <div className={`flex flex-col lg:flex-row lg:gap-6 transition-all duration-300 ${selectedMatchId ? 'items-start' : ''} max-w-full`}>
              {/* Left Column: Match List */}
              <div className={`transition-all duration-300 ${selectedMatchId ? 'hidden lg:block lg:w-[45%]' : 'w-full'} p-4 lg:p-0`}>
                {/* Banner Carousel */}
                {!selectedMatchId && (
                  <div className="relative w-full h-[200px] lg:h-[300px] lg:rounded-lg overflow-hidden mb-6 shadow-2xl -mx-4 lg:mx-0 w-[calc(100%+2rem)] lg:w-full group">
                ...
                <div className="flex bg-brand-surface rounded-full overflow-hidden mb-6 p-1 border border-white/5">
                  <button className={`px-6 py-3 font-black transition-all flex-1 rounded-full ${selectedMatchId ? 'text-[10px]' : 'text-[11px]'} ${!selectedMatchId ? 'bg-brand-primary text-black shadow-lg shadow-green-500/20' : 'text-gray-500 hover:text-white'}`}>
                    Upcoming
                  </button>
                  <button className={`px-6 py-3 font-black flex-1 rounded-full transition-all text-gray-500 hover:text-white ${selectedMatchId ? 'text-[10px]' : 'text-[11px]'}`}>
                    Top Matches
                  </button>
                </div>
                ...
                <div className="bg-brand-surface rounded-2xl overflow-hidden border border-brand-border shadow-xl">
                  ...
                  <div className={`grid bg-[#111111] text-[9px] font-black text-gray-500 h-10 px-6 uppercase tracking-widest items-center border-b border-white/5 rounded-t-2xl ${selectedMatchId ? 'grid-cols-[1.5fr_1.5fr_1fr]' : 'grid-cols-[1.5fr_1.5fr_1fr]'}`}>
                    <div className="text-left font-bold opacity-80 border-r border-white/5 h-full flex items-center">Match Result</div>
                    <div className="text-left font-bold text-center opacity-80 border-r border-white/5 h-full flex items-center justify-center">Double chance</div>
                    <div className="text-right font-bold pr-2 opacity-80 h-full flex items-center justify-end">Both Score</div>
                  </div>
      ...
      {selectedBets.length > 0 && !isBetslipOpen && (
        <div className="fixed bottom-24 right-4 z-[95] lg:hidden">
          ...
          >
            ...
            <span className="text-[8px] font-black uppercase italic mt-1">Ticket</span>
          </button>
        </div>
      )}
